...
5. EB beam chain at [damage-resolver.ts L451-L474](packages/game-server/src/application/services/combat/tabletop/rolls/damage-resolver.ts#L451) preserves `actorId` + `targetId` — beam 2 sees the same Hex effect.

Symptom (`8+0`, `6+0`) means `dmgEffects` is empty at damage time. Most likely suspects (by probability):

- **A (highest) — stale resources stomped by bonus-action patch:** After the BuffDebuff handler writes Hex via `updateCombatantState`, the same `handle()` method at [buff-debuff-spell-delivery-handler.ts L181-L188](packages/game-server/src/application/services/combat/tabletop/spell-delivery/buff-debuff-spell-delivery-handler.ts#L181) patches `bonusActionUsed:true` using `actorCombatant.resources` — **stale pre-effect-write snapshot**. If `patchResources` rewrites the whole resources blob rather than shallow-merging, the freshly persisted Hex effect is overwritten. Must verify `patchResources` semantics in [resource-utils.ts](packages/game-server/src/application/services/combat/helpers/resource-utils.ts).
...
2. **If A:** re-fetch the combatant before merging the `bonusActionUsed` patch, OR merge via a helper that preserves `activeEffects`.
3. **If B:** fix `createEffect` to thread `diceValue` onto the returned `ActiveEffect`.
4. **If C:** guard `initiateSpellCast` against spells with no enemy counterspell opportunities.
5. Extend `warlock/hex-and-blast.json` E2E to assert final HP is in `(beam1_raw + beam2_raw + 2)…(beam1_raw + beam2_raw + 12)` range once fixed.

### Risks
- **Low:** Damage message suffix (`effectBonusSuffix`) is cosmetic — once effects fire, totals correct.
- **Medium (if A):** the fix affects every self+target buff/debuff combo with bonus-action casting (Shield of Faith, Hunter's Mark). Broader E2E sweep needed.
- **Low:** EB chaining shared with Scorching Ray; confirm no regression.
...
2. **QueueableDiceRoller bypass (likely derivative, not primary):** `this.diceRoller.d20()` at [L1144-L1152](packages/game-server/src/application/services/combat/combat-service.ts#L1144) DOES consume from the queue if `diceRoller` is `QueueableDiceRoller`. The CombatService is constructed once with the shared `deps.diceRoller`. Test harness wires QueueableDiceRoller as the singleton. Likely the queue IS being consumed — the author's perception of "queue ignored" is really bug (1) making every save look like a pass regardless of the queued value. Verify via log inspection post-fix.
...
3. **Add a focused unit test**: combatant with known WIS mod + Paralyzed `ActiveEffect` with `saveToEnd:{ability:'wisdom', dc:15}`, `QueueableDiceRoller.queue([10])` → assert logged modifier matches WIS mod AND outcome matches `(10 + mod + prof) vs 15`.
4. **Re-enable W2 Counterspell leg** after the save consumes queued dice deterministically. Assert `spellSlot_3` 2→1.

### Risks
- **Medium:** Signature change touches start-of-turn + end-of-turn callers. `ongoing_damage` and `recurring_temp_hp` paths don't use ability scores, so logically unaffected, but coverage must confirm.
- **Low:** Hold Person save-to-end behavior will change for any scenario relying on the buggy `+0`. None currently do (GAP-8 is documented blocker).
- **Low:** If another wiring path constructs `CombatService` with a non-queue `DiceRoller`, separate fix needed — verify via grep on `new CombatService(`.
...
### Root Cause
Bane is not in the catalog. Grep of `packages/game-server/src/domain/entities/spells/**` finds zero `BANE` references. `resolveSpell('Bane', sheet)` returns no match → delivery dispatch is skipped → fallback `[WARN] Spell 'Bane' has no effects defined` fires in [spell-action-handler.ts L342-L346](packages/game-server/src/application/services/combat/tabletop/spell-action-handler.ts#L342). Concentration never switches because `isConcentration=false` (no catalog match).
...
**Option A (preferred): catalog entry + extend BuffDebuff handler with per-target save-on-cast.**
```ts
export const BANE = {
  name: 'Bane',
  level: 1,
  ...
  school: 'enchantment',
  castingTime: 'action', range: 30,
  components: { v:true, s:true, m:'a drop of blood' },
  classLists: ['Bard','Cleric'],
  description: '...'
} as const satisfies CanonicalSpell;
```
In `BuffDebuffSpellDeliveryHandler.handle`: when `spellMatch.saveAbility` is set AND effect `type` is `'penalty'|'disadvantage'`, run a save per target via `SavingThrowResolver` (or inline). Apply effect only on **failure**.

**Option B:** Add `effectsOnFailure: SpellEffectDef[]` to the canonical spell type and route Bane through `SaveSpellDeliveryHandler` (already iterates targets + applies conditions on failure). Broader handler change, cleaner separation.

### Target cap / multi-target
Bane targets up to 3 (+1 per upcast). Existing `appliesTo:'enemies'` resolution applies to ALL matching combatants with no cap. First shipment: single-target via `castInfo.targetName` (if C2 only uses single-target). Cap support (`maxTargets` field or target-list plumbing) = follow-up.

### Risks
- **Medium (Option A):** extending BuffDebuff handler to run saves affects semantics of future debuff spells. Verify Hold Person still routes to `SaveSpellDeliveryHandler` (it has `damage` + `conditions.onFailure`, no `effects[]` with `appliesTo:'target'`). Check registry order at [spell-action-handler.ts L74-L81](packages/game-server/src/application/services/combat/tabletop/spell-action-handler.ts#L74).
- **Low:** Penalty dice on attack rolls/saves already consumed by `calculateBonusFromEffects` — no AI changes.
- **Medium:** multi-target cap needs new plumbing — defer if single-target suffices for C2.
...
1. **GAP-6 is localisation, not design.** Write the failing unit test first, binary-search A/B/C. Expected fix ≤20 LOC.
2. **GAP-8 needs a contract change** (`processActiveEffectsAtTurnEvent` takes hydrated creatures). Worth the refactor — raw-record ability lookup is the same anti-pattern `hydrateCombat` already solves elsewhere.
3. **GAP-11 is straight additive.** Ship Option A with single-target first. Defer multi-target + `maxTargets` to follow-up.
...
<!-- Superseded Shield-of-Faith research removed. -->
