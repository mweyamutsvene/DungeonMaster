...
6. **Zone Cost Penalties Accumulate Linearly; No Multi-Zone Interaction** — If a cell is in **multiple overlapping zones**, pathfinding applies the penalty **once per zone** (loop iterates all zones and breaks after first match). Correction: **Only applies ONCE per cell** per zone set, but if zones overlap, penalty accumulates. A cell in 3 hazard zones gets +45ft cost (15×3). This is intentional but can create unintended "super-walls" if zones stack. **Fix:** Cap the maximum penalty per cell to a reasonable value (e.g., 30ft), or only apply the strongest penalty (not cumulative).

   *Code location:* `pathfinding.ts` lines ~378-386; zone penalty loop applies per zone

7. **Pit Terrain Elevation/Depth Can Be Undefined or Zero** — When `setTerrainAt()` creates a pit cell without providing `terrainDepth` in options, it defaults to undefined, and `getPitDepthOf()` returns 0. This causes `computePitFallDamage()` to roll 0d6 (no damage). **Fix:** Always pass `terrainDepth` in options when creating pits, or initialize all pits with a default depth (e.g., 10ft) in map creation factory.
