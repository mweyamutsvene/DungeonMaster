...
async function main() {
  ...
  const inputPath = path.resolve(
    repoRoot,
    "RuleBookDocs",
    "Magic Items - D&D Beyond Basic Rules - Dungeons & Dragons - Sources - D&D Beyond.html"
  );
  const outputPath = path.resolve(repoRoot, "RuleBookDocs", "markdown", "magic-items.md");
  ...
  const out = [];
  out.push("# Magic Items");
  out.push("");

  const children = $content.children().toArray();

  // Chunk between top-level H2 sections (safe: never splits tables).
  const sections = [];
  ...
  // Keep each H2 section as its own chunk for readability.
