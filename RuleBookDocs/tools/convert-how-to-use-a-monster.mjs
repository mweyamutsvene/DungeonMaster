...
function renderChild($, child, out) {
  ...
  // Recurse into common container divs to preserve order.
  if (tag === "div") {
...
async function main() {
  ...
  const inputPath = path.resolve(
    repoRoot,
    "RuleBookDocs",
    "How to Use a Monster - D&D Beyond Basic Rules - Dungeons & Dragons - Sources - D&D Beyond.html"
  );
  const outputPath = path.resolve(repoRoot, "RuleBookDocs", "markdown", "how-to-use-a-monster.md");
  ...
  // Two chunks: (Overview + Parts) then (Monster Conversions)
  const chunkGroups = [
    ["Stat Block Overview", "Parts of a Stat Block"],
    ["Monster Conversions"],
  ];
  ...
  for (const group of chunkGroups) {
    ...
    if (groupSections.length) chunks.push(groupSections.join("\n\n"));
  }

  // Any remaining H2 sections (future-proof)
  for (const s of sections) {
    if (!s.title) continue;
    if (emitted.has(s.title)) continue;
    chunks.push(s.md.trim());
    emitted.add(s.title);
  }
